...
## 2026-09-14 | 新机器（D18 密令自动提取 + 网页图片解析）

**完成**
- **D18 密令自动提取**：新建 `server/rag/code_extractor.py`，规则式从攻略文本中识别「密令/兑换码/口令/礼包码/激活码/福利码」引导词附近的码，提取码文本与奖励，去重入库 `codes` 表（不覆盖已有奖励）
  - 引导词：`密令/兑换码/礼包码/口令/激活码/福利码/神秘代码`（避免把普通正文误判为码）
  - 码形：连续中文（3~20 字）或字母数字含空格（2~20 位），含噪音词过滤（`引导词/详情/公告/入口`等）
  - 奖励：码后 60 字内抓「奖励/赠送/礼包/领取…」引导的短语，尽力抽取（运营可在 P3 后台修正）
  - 接入位置：`routes_admin._ingest`（在线导入）、`scripts/build_kb.py`（批量建库）、`routes_chat._stream`（聊天粘链接/附件时自动提取，parsed 事件展示收录条数）
- **D18 网页图片解析**：`parse_url()` 增强
  - 下载 `<img>`（支持 `data-src` 微信懒加载）→ 走 vision/OCR 双通道解析图内文字 → 追加到正文末尾（`[图片·通道]` 前缀）
  - Best-effort：单图失败仅告警，不阻断正文解析；跳过 data URI、重复 URL、小于 3KB 的图标
  - `max_images=8`（默认），支持传参控制；`max_images=0` 关闭图片解析
- **D18 vision 守卫修复**：`VLImageReader.read()` vision_model 为空时给出明确中文错误，不再用空模型名调 API
- **auto 通道回退**：`vision.get_reader("auto")` — vision_model 未配置时自动回退 OCR（不再因 key 存在就强制走 VL）
- **密令聊天可见**：`chat-upload` 解析链接后，在 parsed 事件中追加「密令：已自动收录 N 条…」条目，前端直接展示
- **12 个新 pytest 全过**（`tests/test_code_extractor.py`）：码提取/噪音过滤/奖励截断/去重入库/网页图片解析 mock（含跳过小图/失败图/上限 cap）/vision 守卫；**46 个总测试全过**

**下一步**
- [ ] P2 图片解析：本机拉视觉模型（如 `qwen2.5-vl:7b` 或 `bakllava`，8GB 显存有限注意共享）或装 PaddleOCR（CPU）让图片解析真正可跑
- [ ] 用户把真实攻略放入 `knowledge/raw/` 后 `scripts/build_kb.py` 重建（重建时密令自动提取入库）
- [ ] 管理后台 `http://127.0.0.1:19310/admin` 体验（知识库/密令/运维/模型）
...
**完成**
- `web/index.html` 重做视觉与布局（JS 功能逻辑保持不变，全部沿用已修复的 event/data 配对 SSE 解析）
- 游戏风：深紫蓝星夜渐变背景 + 金色描边 + 发光，标题金字渐变，金色发送按钮，AI 气泡带 🐌 角标，loading 🎁 弹跳动画，密令卡片金色发光 hover，附件/tag 金系配色
- 响应式（@media 640px）：消息宽度收窄、角标隐藏、输入/发送大点击区、toolrow 换行、urlInput 全宽、腰包 safe-area 适配
- node --check 通过，页面线上可访问（200）

**下一步**
- [ ] 用户确认效果（桌面 + 手机都看下）
- [ ] 管理后台 admin 页是否也做响应式/游戏风？待定
- [ ] 其余同前（补充攻略 / QQ 渠道）

## 2026-09-09 | 开发机（SSE 事件解析最终修复：出处有、内容无）

**问题升级**：强刷后密令"只有出处没有内容"。
**根因**：SSE 事件类型在 `event:` 行，data 负载 `{"content":...}`；前端却按 `evt.delta`/`evt.parsed`/`evt.error`（payload 里不存在的键）判断 → sources（数组特判）能显示，其余事件全部漏。实际此前所有内容渲染都靠旧前端畸形逻辑/未真正经过前端验证。
**修复**：`processBlock` 改为标准 SSE 配对解析——取 `event:` 行做事件名 + `data:` 行做 payload（sources/delta/parsed/error/done 全走）。node --check 验证通过，30 pytest 全过。
**教训**：SSE 消费必须按 `event:` + `data:` 配对解析，不能靠 payload 键猜事件类型；每次改前端都应 node --check。
